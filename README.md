...
- 기본 로컬 provider를 Turbo Engine: MLX-LM(`jedisct1/gemma-4-12B-it-txt-mlx-8bit`)으로 두고, SGLang과 Ollama Gemma(`gemma4:12b`)를 선택 옵션으로 유지했습니다.
...
- **Turbo Engine: MLX-LM**: macOS 로컬 Gemma 4 12B 기본 provider입니다. 앱이 `mlx_lm.server`를 `enable_thinking:false`로 자동 확인/기동해 짧은 요청에서 불필요한 reasoning 토큰을 줄입니다.
- **SGLang 선택 옵션**: SGLang/MLX 서버도 선택지로 남겨 두지만, 현재 Gemma 4 12B MLX 백엔드 조합은 로컬 검증에서 기동 실패가 확인되어 기본값으로 쓰지 않습니다.
- **빠른 시작 fallback**: MLX-LM 또는 SGLang이 첫 실행 다운로드나 워밍업 중이면 앱은 Ollama `gemma4:12b`가 준비된 경우 즉시 그쪽으로 Kun을 시작하고, 선택한 로컬 엔진은 다음 실행을 위해 계속 준비합니다.
- **Ollama Gemma 선택 옵션**: MLX-LM/SGLang 대신 Ollama의 `gemma4:12b` 모델을 선택할 수 있습니다.
...
- `mlx-lm-gemma`: 기본 로컬 MLX-LM Gemma 4 12B 방식
- `sglang-gemma`: 선택 가능한 SGLang Gemma 4 12B 방식
