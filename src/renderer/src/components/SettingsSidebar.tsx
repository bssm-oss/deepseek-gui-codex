...
import { Bot, ChevronLeft, FlaskConical, Globe, Keyboard, PencilLine, Settings, Smartphone } from 'lucide-react'
  ...
  return (
    <aside className="ds-drag flex w-[248px] shrink-0 flex-col border-r border-ds-border bg-ds-sidebar backdrop-blur-md">
      ...
      <nav className="ds-no-drag flex flex-col gap-0.5 px-2">
        ...
        <button type="button" className={catCls('testsprite')} onClick={() => setCategory('testsprite')}>
          <FlaskConical className="h-4 w-4 shrink-0 opacity-70" strokeWidth={1.75} />
          {t('testSprite')}
        </button>
